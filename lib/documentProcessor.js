...
export class DocumentProcessor {
  ...
  async withTimeout(promise, timeoutMs = this.apiTimeout, retries = this.maxRetries, operation = 'API call',  signal = null) {
    if (signal?.aborted) {
        throw new Error(`Operation '${operation}' cancelled by user.`);
    }
  ...
  // REPLACE the existing generateSummary method in your DocumentProcessor class
  async generateSummary(text, detectedLanguage = 'en', persona = 'General User') {
    ...
    try {
      ...
      const languageInstruction = detectedLanguage !== 'en'
        ? `YOU MUST RESPOND ENTIRELY IN ${this.getLanguageName(detectedLanguage).toUpperCase()}.`
        : `RESPOND IN ENGLISH.`;

      // INNOVATION: Persona-aware Chain of Thought Prompting
      const prompt = `${languageInstruction}

Role: Expert Legal Aide acting for a ${persona}.
Task: Analyze this document to protect the ${persona}'s interests.

CHAIN OF THOUGHT:
1. Identify the Document Type (e.g., Rental Agreement, NDA).
2. Scan for specific monetary figures, dates, and liability clauses.
3. Critically evaluate clauses that are unfair to a ${persona} (e.g., if Tenant, look for eviction without notice).
4. Extract key entities (Landlord, Tenant, etc.).

OUTPUT INSTRUCTIONS:
Create a structured summary using EXACTLY this markdown format:

Main Facts:
- **Document Type:** [type]
- **Key Financials:** [Rent, Deposit, Salaries, etc.]
- **Critical Dates:** [Start, End, Deadlines]

## Parties Involved
**[Party 1 Name]:** [Role & Responsibilities]
**[Party 2 Name]:** [Role & Responsibilities]

## Rights and Obligations (${persona} Perspective)
...
## Critical Risk Assessment for ${persona}
- [Risk 1 - e.g., "High Penalty"]
- [Risk 2 - e.g., "Unilateral Changes"]

Document Text (first 6000 chars):
${text.substring(0, 6000)}
`;

      console.log(`Generating structured summary in ${detectedLanguage} for ${persona}...`);
      const result = await this.withTimeout(
      ...
      const hasStructure = generatedSummary.includes('##') || generatedSummary.includes('**');
...
`;
  ...
  async answerQuestion(question, documentText, language = 'en') {
    try {
      ...
      const languageInstruction = `Answer entirely in ${this.getLanguageName(language)}.`;
